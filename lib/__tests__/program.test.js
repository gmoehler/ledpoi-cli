...
const cpoi_inval_cmd = "./lib/__tests__/testprog_inval_cmd.cpoi";
const cpoi_inval_len = "./lib/__tests__/testprog_inval_len.cpoi";

test("start, stop, sync program", () => {
	prg.startProgram(client);
	prg.pauseProcessing(client);
	prg.stopProcessing(client); // calls status twice
	prg.syncProgram(client, 3);
	
	expect(client.sendCmd).toHaveBeenCalledTimes(6);
	...
	expect(client.sendCmd).toHaveBeenCalledWith([231, 0, 0, 0, 0, 0]);
});
...
test("failure uploading program with illegal cmds", async () => {
	expect.assertions(1);
	try {
		await prg.uploadPrograms(client, cpoi_inval_cmd);
	} catch (e) {
    	expect(e.message).toMatch(/Error in program./);
	}
});

test("uploadProgram with too many prog steps", async () => {
	expect.assertions(1);
	try {
		await prg.uploadPrograms(client, cpoi_inval_len);
	} catch (e) {
    	expect(e.message).toMatch(/Error in program./);
	}
});
