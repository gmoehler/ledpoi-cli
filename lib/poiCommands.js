...
const prog = [];
let _loopIncr = 0; // id of first loop (added to id given by user)
let _loopHi = 0; // highest loop id
...
global.dim = function(factor) {
	const val = u8c(factor * 100);
	prog.push([204, val, 0, 0, 0, 0]);
}

global.loopStart = function(loopId, numLoops) {
	const numL = splitUint16(numLoops);
	_loopHi = loopId+_loopIncr;
	prog.push([214, u8c(_loopHi), 0, 0, numL.u, numL.l]);
}
