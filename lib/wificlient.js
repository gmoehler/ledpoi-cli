...
module.exports = class WifiClient {
	...
	init() {	
		...
		this.client.on('connect', () => {
			console.log(`Connected to ${this.connectionParams.host}:${this.connectionParams.port}.`);
			this.connected = true;
		});


		this.client.on('end', () => {
			console.log(`Connection ended to ${this.connectionParams.host}:${this.connectionParams.port}.`);
			this.connected = false;
			clearInterval(this.keepAliveInterval);
		});

		this.client.on('close', () => {
			console.log(`Connection closed from to ${this.connectionParams.host}:${this.connectionParams.port}.`);
			this.connected = false;
			clearInterval(this.keepAliveInterval);
			this.client.destroy();
		});
